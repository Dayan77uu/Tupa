...
export function ProcedureTracking() {
  const { id } = useParams();
  const location = useLocation();

  // Determine if this is student or administrative view
  const isAdminView = location.pathname.includes("/office/");

  // Simulate different status for demo: "observed", "approved", "review"
  const currentStatus = "observed";
  ...
  const documents = [
    ...
    { id: 2, name: "Copia del DNI", fileName: "dni-carlos-quispe.pdf", status: "observed" },
  ...
  const historyEvents = [
    {
      ...
      time: "14:20",
      status: "Observado",
      from: "Oficina de Registro Académico",
      to: "Estudiante",
      responsible: "Ana Huanca Torres",
      comment: "DNI adjunto vencido. Se requiere documento vigente.",
    },
    {
      type: "forwarded",
          ...
          <AlertDescription>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <p className="font-semibold text-[#92400E] mb-1">Tu expediente tiene observaciones</p>
                <p className="text-sm text-[#92400E]">{expedient.observationText}</p>
              </div>
              {!isAdminView && (
                <Button className="bg-[#F59E0B] hover:bg-[#D97706] text-white flex-shrink-0">
                  <Upload className="h-4 w-4 mr-2" />
                  Subsanar documentos ahora
                </Button>
              )}
            </div>
          </AlertDescription>
      ...
      {currentStatus === "approved" && (
        <Alert className="bg-[#D1FAE5] border-[#6EE7B7]">
          ...
          <AlertDescription>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <p className="font-semibold text-[#065F46] mb-1">¡Tu expediente ha sido aprobado!</p>
                <p className="text-sm text-[#065F46]">
                  Tu trámite ha sido completado satisfactoriamente. Descarga tu resultado.
                </p>
              </div>
