"""
Fase 3 (Sprint 6): inserta tramites NUEVOS en tcatalogotramite -- trámites del
CSV oficial del TUPA que no existen en ninguna forma en la BD real (no
compitieron por un match de texto con ningun trámite existente en el rango de
score confiable, o son variantes claramente distintas -maestria/doctorado,
segunda especialidad- de un tramite ya existente).
...
Nota: 4 de estos 31 tienen como oficina de PRESENTACION "Secretaria de la Sede
Sicuani" (Filial Sicuani), que no esta en la lista de 13 oficinas autorizadas
por el usuario -- se inserta el tramite igual (oficina_responsable si esta en
la lista), pero se omite su fila de tflujoderivacion (sin inventar oficina).

Uso: backend/venv/Scripts/python.exe -m scripts.cargar_fase3_nuevos
"""
import io
import json
...
FASE3_PATH = (
    ...
    r"dc79bd91-2e5b-4965-a8ac-86262cb1be01/scratchpad/fase3.json"
)
...
def run():
    with io.open(FASE3_PATH, encoding="utf-8") as f:
        filas = json.load(f)
    ...
    with app.app_context():
        ...
        print("\n" + "=" * 60)
        print(f"Tramites insertados (Fase 3): {len(insertados)}")
