...
UPDATE texpediente e SET nidtoficinaactual = ut.oficina FROM (
  ...
  FROM tunidadtramite
  GROUP BY ccodigo
) ut WHERE ut.ccodigo = e.ccodigo AND e.nidtoficinaactual IS NULL;
...
ALTER TABLE texpediente ALTER COLUMN nidtoficinaactual TYPE INTEGER, ALTER COLUMN nidtoficinaactual SET NOT NULL;

-- ---------------------------------------------------------------------------------------
-- 2. Motivo de rechazo del voucher (faltaba, RF13/HU-12)
-- ---------------------------------------------------------------------------------------
ALTER TABLE texpediente
  ADD COLUMN IF NOT EXISTS cmotivorechazovoucher VARCHAR(255) NULL;
...
VALUES ('PA88401484E', 1, 2) ON CONFLICT DO NOTHING;
